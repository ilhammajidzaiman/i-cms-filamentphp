            ...
            <div class="w-full grid grid-cols-12 gap-12">
                <div class="w-full col-span-full md:col-span-8 space-y-4">
                    <div class="flex items-center space-x-2 text-slate-500">
                        ...
                        <a wire:navigate href="{{ route('partner.index') }}"class="whitespace-nowrap hover:underline">
                            {{ Str::ucfirst(__('mitra')) }}
                        </a>
                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5"
                            stroke="currentColor" class="size-5">
                            <path stroke-linecap="round" stroke-linejoin="round" d="m8.25 4.5 7.5 7.5-7.5 7.5" />
                        </svg>
                        <a wire:navigate href="{{ route('partner.index') }}"class=" hover:underline line-clamp-1">
                            {{ $record->title ?? null }}
                        </a>
                    </div>
                    ...
                    <div class="sticky top-[30%] self-start space-y-8">
                        ...
                        <div class="space-y-2">
                            <h1 class="text-2xl font-semibold">
                                {{ Str::ucfirst(__('mitra lainnya')) }}
                            </h1>
                            <div class="w-12 h-1 rounded-full bg-sky-500 "></div>
                            @foreach ($other as $item)
                                <h3 class="line-clamp-1">
                                    <a href="{{ route('partner.show', $item->slug) }}"
